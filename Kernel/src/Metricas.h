...
typedef struct{
	...
	t_dictionary* memory_load; // (por cada memoria):  Cantidad de INSERT / SELECT que se ejecutaron en esa memoria respecto de las operaciones totales (en los últimos 30 segundos).
}t_metricas;

typedef struct{
	unsigned long long inicio;
	unsigned long long fin;
	unsigned int nro_memoria;
}t_metrica_operacion;
